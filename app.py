...
if selected_sport == "축구":
    ...
    selected_leagues = [lid for lid, selected in zip(["2","3","10","39","140","135","78"], 
                                                     [l_2, l_3, l_10, l_39, l_140, l_135, l_78]) if selected]
    LEAGUE_MAP = {"2":"챔피언스리그", "3":"유로파리그", "10":"A매치", "39":"프리미어리그", "140":"라리가", "135":"세리에A", "78":"분데스리가"}
    ...
    if analyze_button:
        ...
        for idx, league_id in enumerate(selected_leagues):
            status_text.text(f"🔍 {LEAGUE_MAP[league_id]} 정밀 데이터 스캔 중... ({idx+1}/{len(selected_leagues)})")
            ...
            try:
                ...
                for match in res.get('response', []):
                    ...
                    if is_live and elapsed_time: top_display = f"{LEAGUE_MAP[league_id]} ({match_time}) <br><span style='color:#ff5252; font-size:12px;'>[진행중: {elapsed_time}분]</span>"
                    elif is_finished: top_display = f"{LEAGUE_MAP[league_id]} ({match_time}) <br><span style='color:#aaa; font-size:12px;'>[경기 종료]</span>"
                    else: top_display = f"{LEAGUE_MAP[league_id]} ({match_time})"
                    ...
                    match_display = f"<div class='match-box'><div class='team-side home-side'><img src='{home_logo}' class='team-logo'><div class='team-name'>{home_kr}</div></div><div class='score-side' style='color:{score_color};'>{score_text}</div><div class='team-side away-side'><div class='team-name'>{away_kr}</div><img src='{away_logo}' class='team-logo'></div></div>"
                    ...
                    stat_box = f"<span style='color:#aaa;'>최종 전력 분석 파워:</span><br>{home_kr} <b>{int(h_power)}점</b> vs <b>{int(a_power)}점</b> {away_kr}"
                    ...
                    new_data_list.append({"sport": "축구", "league": top_display, "match_display": match_display, "stat_box": stat_box, "referee": "👨‍⚖️ 주심 배정 완료", "venue": "🏟️ 구장 컨디션 양호", "p_h": p_h, "p_d": p_d, "p_a": p_a, "win_pick": win_pick, "pick_color": pick_color, "ou_color": ou_color, "control_pick": "전력 및 상대 전적 기반 분석 완료", "over_under": over_under, "radar_html": radar_html, "lineup_html": "<div style='text-align:center; color:#888;'>축구 라인업 제공 준비 중</div>", "detail_html": ""})
            except: pass
    ...
    if analyze_button:
        ...
        try:
            ...
            for idx, game in enumerate(all_games):
                ...
                ou_color = "#FFF59D" if status_code == 'Final' else "#ddd"
                ...
                new_data_list.append({"sport": "야구", "league": top_display, "match_display": match_display, "stat_box": stat_box, "referee": f"投: {home_pitcher}({h_p_hand}) vs {away_pitcher}({a_p_hand})", "venue": venue, "p_h": f"{h_win_prob:.0f}", "p_d": "0", "p_a": f"{a_win_prob:.0f}", "win_pick": win_pick, "pick_color": pick_color, "ou_color": ou_color, "control_pick": "좌우 상성 및 기세(30%) 가중치 적용 완료", "over_under": ou_text, "lineup_html": lineup_html, "detail_html": detail_html, "radar_html": ""})
            st.session_state['analyzed_data_list'] = new_data_list
        except: pass
    ...
    for idx, data in enumerate(st.session_state['analyzed_data_list']):
        with cols[idx % 3]:
            ...
            with st.expander("🔍 상세 지표 & 선발 명단 확인"):
                if data['detail_html']: st.markdown(data['detail_html'], unsafe_allow_html=True)
                if data['sport'] == "축구" and data.get('radar_html'): st.markdown(data['radar_html'], unsafe_allow_html=True)
                if data['lineup_html']: st.markdown(data['lineup_html'], unsafe_allow_html=True)
            st.write("")
