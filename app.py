...
# 🎨 UI CSS: 박스 크기 고정 및 말줄임표(ellipsis) 적용
custom_css = """
<style>
...
/* 💡 핵심: AI 분석 글자 1줄 고정 및 말줄임표 처리 */
.ai-advice {
    font-size: 11.5px; 
    color: #999; 
    font-weight: normal; 
    white-space: nowrap; /* 절대 줄바꿈 금지 */
    overflow: hidden;    /* 박스 넘어가는 글자 숨김 */
...
# 💡 핵심: 번역기 오류 방어막 (실패 시 0.5초 쉬고 재도전)
@st.cache_data(show_spinner=False)
def translate_to_ko(text):
    if not text or text == 'N/A': return '데이터 분석 중'
    try: 
                ...
                if h_power < 10 or a_power < 10:
                    ...
                    if odds_h > 0 and odds_a > 0:
                        if odds_h < odds_a - 0.3:
                            if a_power > h_power + 35: pred_winner, win_pick, pick_color = "away", f"🚨 데이터 역배픽: {away_kr} 승", "#ff5252"
                            else: pred_winner, win_pick, pick_color = "home", f"🟢 {home_kr} 승 (정배당)", "#00E676"
                        elif odds_a < odds_h - 0.3:
                            if h_power > a_power + 35: pred_winner, win_pick, pick_color = "home", f"🚨 데이터 역배픽: {home_kr} 승", "#ff5252"
            ...
            <div style="height: 100%;">
                <div class='card-box'>
                    ...
                    <div class='predict-txt'>
                        <div style='color: {data['pick_color']}; margin-bottom: 3px;'>🎯 {data['win_pick']}</div>
