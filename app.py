...
# ==========================================
# 3. 언어 번역 모듈 (💡 딕셔너리를 함수 안으로 이동하여 캐시 충돌 방지)
# ==========================================
@st.cache_data(show_spinner=False)
def translate_to_ko(text):
    if not text or str(text).strip() in ['', 'N/A']: return '미정'
    
    # 캐싱 에러를 막기 위해 글로벌 변수 대신 지역 변수로 선언
    custom_dict = {
...
# ==========================================
# 5. API Fetcher (💡 HEADERS를 내부로 이동하여 캐시 충돌 100% 방지)
# ==========================================
@st.cache_data(ttl=600, show_spinner=False)
def fetch_api_football_fixtures(league_id, season, date_str):
    api_key = st.secrets.get("FOOTBALL_API_KEY", "")
    local_headers = {'x-apisports-key': api_key} if api_key else {}
    try: 
        res = requests.get("https://v3.football.api-sports.io/fixtures", headers=local_headers, params={"league": league_id, "season": season, "date": date_str, "timezone": "Asia/Seoul"}, timeout=10)
...
def fetch_api_football_standings(league_id, season):
    ...
    local_headers = {'x-apisports-key': api_key} if api_key else {}
    try:
        res = requests.get("https://v3.football.api-sports.io/standings", headers=local_headers, params={"league": league_id, "season": season}, timeout=10)
...
def generate_football_advanced_stats(h_team, a_team, h_prob, is_finished, h_score=0, a_score=0):
    ...
    # 💡 숫자가 아닌 None이 들어왔을 때를 대비한 0.0 처리 방어막
...
def get_prediction_and_commentary(home_kr, away_kr, h_prob, h_gf, a_gf, is_finished, h_score, a_score):
    ...
    # 승무패
    if h_prob > a_prob + 10 and h_prob > d_prob: pred_win = "home"; win_txt = f"🟢 {home_kr} 승리 유력"
    elif a_prob > h_prob + 10 and a_prob > d_prob: pred_win = "away"; win_txt = f"🔵 {away_kr} 승리 유력"
    else: pred_win = "draw"; win_txt = "🟡 치열한 접전 (무승부 가능성)"
    
    # 핸디캡
    if h_prob > 60: pred_handi = "home"; handi_txt = f"💪 {home_kr} 2골 차 이상 대승 기대"
    elif a_prob > 60: pred_handi = "away"; handi_txt = f"💪 {away_kr} 2골 차 이상 대승 기대"
    elif pred_win == "home": pred_handi = "away"; handi_txt = f"🛡️ {away_kr}의 탄탄한 방어 (1골 차 승부)"
    else: pred_handi = "home"; handi_txt = f"🛡️ {home_kr}의 탄탄한 방어 (1골 차 승부)"
...
# 7. 메인 UI (사이드바)
# ==========================================
st.markdown("<h1 style='text-align: center; color: #00E676; font-size: 28px; margin-bottom: 30px;'>🏆 종합 스포츠 AI 분석실 (V69 무결점 완벽판)</h1>", unsafe_allow_html=True)
            ...
            if matches:
                for match in matches:
                    try:
                        ...
                        h_print = h_score if h_score is not None else 0
                        a_print = a_score if a_score is not None else 0
