...
CUSTOM_DICT = {
    ...
    "Boston Celtics": "보스턴", "Dallas Mavericks": "댈러스", "Denver Nuggets": "덴버", "Minnesota Timberwolves": "미네소타", 
    "Oklahoma City Thunder": "오클라호마시티", "New York Knicks": "뉴욕 닉스", "Indiana Pacers": "인디애나", "Los Angeles Lakers": "LA 레이커스", 
    "Golden State Warriors": "골든스테이트", "Miami Heat": "마이애미", "Philadelphia 76ers": "필라델피아", "Milwaukee Bucks": "밀워키", 
    "Phoenix Suns": "피닉스", "LA Clippers": "LA 클리퍼스", "Los Angeles Clippers": "LA 클리퍼스", "Sacramento Kings": "새크라멘토", 
    "New Orleans Pelicans": "뉴올리언스", "Cleveland Cavaliers": "클리블랜드", "Orlando Magic": "올랜도", "Chicago Bulls": "시카고", 
    "Atlanta Hawks": "애틀랜타", "Brooklyn Nets": "브루클린", "Toronto Raptors": "토론토", "Washington Wizards": "워싱턴", 
    "Charlotte Hornets": "샬럿", "Detroit Pistons": "디트로이트", "San Antonio Spurs": "샌안토니오", "Houston Rockets": "휴스턴", 
    "Memphis Grizzlies": "멤피스", "Utah Jazz": "유타", "Portland Trail Blazers": "포틀랜드"
}
...
def create_html_radar(h_vals, a_vals, home_kr, away_kr, is_custom=False):
    ...
    badge = "<div style='color:#ff9800; font-size:11px; margin-bottom:5px;'>⚙️ 전력 분석망 가동</div>"
...
def load_mlb_all_data():
    try:
        h_splits = requests.get("[https://statsapi.mlb.com/api/v1/stats?stats=season&group=hitting&gameType=R&season=2026&playerPool=ALL&limit=1500](https://statsapi.mlb.com/api/v1/stats?stats=season&group=hitting&gameType=R&season=2026&playerPool=ALL&limit=1500)").json()['stats'][0]['splits']
        ...
        p_splits = requests.get("[https://statsapi.mlb.com/api/v1/stats?stats=season&group=pitching&gameType=R&season=2026&playerPool=ALL&limit=1500](https://statsapi.mlb.com/api/v1/stats?stats=season&group=pitching&gameType=R&season=2026&playerPool=ALL&limit=1500)").json()['stats'][0]['splits']
...
def load_mlb_team_momentum():
    try:
        res = requests.get("[https://statsapi.mlb.com/api/v1/standings?leagueId=103,104](https://statsapi.mlb.com/api/v1/standings?leagueId=103,104)", timeout=5).json()
        l10_dict = {}
...
def load_mlb_live_lineup(game_pk, home_pitcher_id, away_pitcher_id):
    try:
        res = requests.get(f"[https://statsapi.mlb.com/api/v1/game/](https://statsapi.mlb.com/api/v1/game/){game_pk}/boxscore").json()
...
# ==========================================
# 🏀 농구(NBA) 전용 무료 API (수비/턴오버 등 정밀 시뮬레이터)
# ==========================================
def load_nba_games_free(date_obj):
    ...
    seen_ids = set()
    for d in [d1, d2]:
        url = f"[https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard?dates=](https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard?dates=){d}"
        try:
            ...
            for ev in res.get('events', []):
                ...
                if kst_time.date() == date_obj:
                    valid_events.append((ev, kst_time))
        except: pass
    return valid_events

def get_nba_details_html(event_id, h_team_name, a_team_name):
    url = f"[https://site.api.espn.com/apis/site/v2/sports/basketball/nba/summary?event=](https://site.api.espn.com/apis/site/v2/sports/basketball/nba/summary?event=){event_id}"
    try:
        res = requests.get(url, timeout=5).json()
        box_players = res.get('boxscore', {}).get('players', [])
        h_strs = []; a_strs = []
        ...
        box_teams = res.get('boxscore', {}).get('teams', [])
        stat_html = ""
        if box_teams and len(box_teams) >= 2:
            a_team_stats = box_teams[0].get('statistics', [])
            h_team_stats = box_teams[1].get('statistics', [])
            def ext(stats, name): return next((s['displayValue'] for s in stats if s['name'] == name), "-")
            
            stat_html = f"""<div class='table-wrapper'><table class='detail-table'>
            <tr><th style='color:#4FC3F7;'>{h_team_name}</th><th style='color:#aaa;'>당일 주요 스탯</th><th style='color:#EF5350;'>{a_team_name}</th></tr>
            <tr><td>{ext(h_team_stats, 'fieldGoalsMade-fieldGoalsAttempted')}</td><td>야투율 (FG)</td><td>{ext(a_team_stats, 'fieldGoalsMade-fieldGoalsAttempted')}</td></tr>
            <tr><td>{ext(h_team_stats, 'threePointFieldGoalsMade-threePointFieldGoalsAttempted')}</td><td>3점슛 (3PT)</td><td>{ext(a_team_stats, 'threePointFieldGoalsMade-threePointFieldGoalsAttempted')}</td></tr>
            <tr><td>{ext(h_team_stats, 'rebounds')}</td><td>리바운드</td><td>{ext(a_team_stats, 'rebounds')}</td></tr>
            <tr><td>{ext(h_team_stats, 'turnovers')}</td><td>턴오버</td><td>{ext(a_team_stats, 'turnovers')}</td></tr>
            </table></div>"""
            
        if not stat_html: stat_html = "<div style='text-align:center; padding:10px; color:#888;'>상세 지표 수집 중... (경기 시작 전)</div>"

        return stat_html, lineup_html
    except: return "<div style='text-align:center; padding:10px; color:#888;'>데이터 연동 대기 중</div>", ""

def run_nba_deep_simulation(h_ppg, h_opp_ppg, a_ppg, a_opp_ppg, ou_line, home_spread_margin, num_sims=5000):
    # 경기 전 입력받은 팀 평균 득/실점을 바탕으로 마진과 총점을 계산하는 정밀 몬테카를로
    exp_h = (h_ppg + a_opp_ppg) / 2
    exp_a = (a_ppg + h_opp_ppg) / 2
    
    # 만약 베가스 기준점이 있으면 시뮬레이션 결과에 약간의 가중치 부여
    if ou_line > 100:
        exp_h = (exp_h * 0.7) + (((ou_line / 2) + (home_spread_margin / 2)) * 0.3)
        exp_a = (exp_a * 0.7) + (((ou_line / 2) - (home_spread_margin / 2)) * 0.3)
    
    h_wins = 0; a_wins = 0; h_handi_wins = 0; o_wins = 0
    for _ in range(num_sims):
        h_s = random.gauss(exp_h, 8.5)
        a_s = random.gauss(exp_a, 8.5)
        if h_s > a_s: h_wins += 1
        else: a_wins += 1
        
        if (h_s - a_s) > home_spread_margin: h_handi_wins += 1
        if (h_s + a_s) > ou_line: o_wins += 1
...
# 📺 메인 UI 렌더링 시작
# ==========================================
st.markdown("<h1 style='text-align: center; color: #00E676; font-size: 28px; margin-bottom: 30px;'>🏆 AI 종합 스포츠 분석실 PRO MAX (V34.0)</h1>", unsafe_allow_html=True)
...
# ==========================================
# ⚽ 축구 로직 (완전 복구됨 - 6월이므로 월드컵/친선전 기본 체크)
# ==========================================
if selected_sport == "축구":
    ...
    if analyze_button:
        ...
        for idx, league_id in enumerate(selected_leagues):
            ...
            querystring = {"league": league_id, "season": str(selected_date.year if selected_date.month > 7 or league_id in ["1", "10"] else selected_date.year - 1), "date": selected_date.strftime('%Y-%m-%d'), "timezone": "Asia/Seoul"}
            
            try:
                res = requests.get("[https://v3.football.api-sports.io/fixtures](https://v3.football.api-sports.io/fixtures)", headers=HEADERS, params=querystring, timeout=10).json()
                for match in res.get('response', []):
                    ...
                    pred_data = requests.get("[https://v3.football.api-sports.io/predictions](https://v3.football.api-sports.io/predictions)", headers=HEADERS, params={"fixture": fix_id}).json().get('response', [])
                    if not pred_data: continue
                    pred = pred_data[0]; comp = pred.get('comparison', {})
                    odds_res = requests.get("[https://v3.football.api-sports.io/odds](https://v3.football.api-sports.io/odds)", headers=HEADERS, params={"fixture": fix_id}).json().get('response', [])
                    lineup_data = requests.get("[https://v3.football.api-sports.io/fixtures/lineups](https://v3.football.api-sports.io/fixtures/lineups)", headers=HEADERS, params={"fixture": fix_id}).json().get('response', [])
                    inj_res = requests.get("[https://v3.football.api-sports.io/injuries](https://v3.football.api-sports.io/injuries)", headers=HEADERS, params={"fixture": fix_id}).json().get('response', [])
                    ...
                    ou_line = 2.5
                    if under_over_val:
                        pred_is_over = '+' in under_over_val
        ...
        if not new_data_list: st.info(f"선택하신 리그에 {selected_date} 일자로 배정된 경기가 없습니다.")
...
# ==========================================
    ...
    with st.sidebar.expander("미국 야구 (MLB 자동 분석)", expanded=True): c_mlb = st.checkbox("메이저리그 (MLB)", value=True)
    ...
    if analyze_button:
        ...
        # 🇰🇷 🇯🇵 KBO/NPB 반자동 로직
        if c_kbo or c_npb:
            BASEBALL_URL = "[https://v1.baseball.api-sports.io/](https://v1.baseball.api-sports.io/)"
            api_leagues = []
            ...
            for idx, (l_id, l_name) in enumerate(api_leagues):
                ...
                try:
                    ...
                    for match in res.get('response', []):
                        ...
                        match_display = f"<div class='match-box'><div class='team-side home-side'><div class='team-name' title='{home_kr}'>{home_kr}</div><img src='{match['teams']['home']['logo']}' class='team-logo'></div><div class='score-side' style='color:{score_color};'>{score_text}</div><div class='team-side away-side'><img src='{match['teams']['away']['logo']}' class='team-logo'><div class='team-name' title='{away_kr}'>{away_kr}</div></div></div>"
        ...
        if not st.session_state['analyzed_data_list'] and not st.session_state['kbo_npb_data_list']: st.info(f"해당 날짜({selected_date})에 한국시간 기준으로 야구 경기가 없습니다.")
        progress_bar.progress(1.0); status_text.text("✅ 야구 스캔 완료!"); time.sleep(1); status_text.empty(); progress_bar.empty()

# ==========================================
# 🏀 농구 (무료 실시간 ESPN API - 핸디캡/언오버 및 라인업 스탯 100% 통합)
# ==========================================
    ...
    if analyze_button:
        ...
        for idx, (event, kst_time) in enumerate(events):
            ...
            h_name_en = h_team['team']['displayName']; a_name_en = a_team['team']['displayName']
            ...
            if status == 'post':
                top_display = f"NBA ({match_time}) <br><span style='color:#aaa; font-size:12px;'>[경기 종료]</span>"
                score_color = "#00E676"; score_text = f"{h_score}:{a_score}"
            elif status == 'in':
                top_display = f"NBA ({match_time}) <br><span style='color:#ff5252; font-size:12px;'>[{status_detail}]</span>"
                score_color = "#ff5252"; score_text = f"{h_score}:{a_score}"
            else:
                top_display = f"NBA ({match_time})"
                score_color = "#888888"; score_text = "VS"
            ...
            if details != 'N/A' and details != 'EVEN':
                ...
                if len(parts) == 2:
                    ...
                    if fav_abbr == h_team['team'].get('abbreviation', ''): home_spread_margin = -val
                    else: home_spread_margin = val
                        
            h_record = h_team.get('records', [{}])[0].get('summary', '0-0').split('-')
            a_record = a_team.get('records', [{}])[0].get('summary', '0-0').split('-')
            h_pct = int(h_record[0]) / max((int(h_record[0]) + int(h_record[1])), 1)
            a_pct = int(a_record[0]) / max((int(a_record[0]) + int(a_record[1])), 1)
            ...
            if status == 'post':
                actual_total = h_score + a_score
                is_over = actual_total > ou_line
                pred_over = total_exp > ou_line
                ...
                actual_home_cover = (actual_margin + home_spread_margin) > 0
                pred_home_cover = (ai_margin + home_spread_margin) > 0
                if actual_home_cover == pred_home_cover: handi_pick += " (적중)"; handi_color = "#B39DDB" 
                else: handi_pick += " (미적중)"; handi_color = "#F48FB1"

            # 경기 시작 전/후에 따라 표시 정보를 다르게 합니다.
            if status == 'pre': # 예정 경기일 경우 반자동 시스템으로 토스
    ...
    for idx, data in enumerate(st.session_state['kbo_npb_data_list']):
        with cols[idx % 3]:
            ...
            with st.expander("🔬 반자동 시뮬레이터 가동", expanded=True):
                ...
                st.markdown(f"""
                <div class='sim-box'>
                    <div style='text-align:center; font-weight:bold; font-size:14px; margin-bottom:8px;'>📊 실시간 역산 결과</div>
        ...
        with cols[idx % 3]:
            ...
            with st.expander("🔬 NBA 정밀 시뮬레이터 가동", expanded=True):
                ...
                <div class='sim-box'>
                    ...
                    <div style='border-top:1px dashed #555; padding-top:10px; text-align:center;'>
                        <div style='color:{pick_color}; font-weight:bold; font-size:13.5px; margin-bottom:5px;'>{win_pick}</div>
                        <div style='color:#B39DDB; font-weight:bold; font-size:13.5px; margin-bottom:5px;'>{handi_pick}</div>
                        <div style='color:#FFF59D; font-weight:bold; font-size:13.5px;'>{ou_text}</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            st.write("")
