...
.match-txt { color: #ffffff; font-size: 18px; font-weight: bold; text-align: center; margin-bottom: 15px; line-height: 1.4; }
...
if st.sidebar.button("🚀 데이터 불러오기 및 AI 분석"):
    ...
    for idx, league_id in enumerate(selected_leagues):
        status_text.text(f"🔍 {LEAGUE_MAP[league_id]} 모든 경기 매치업 수집 중...")
        ...
        try:
            ...
            for match in fixtures:
                ...
                # 💡 핵심 무기: 무조건 모든 대진을 좌우 한 줄 정렬로 빌드
                ...
                # 스탯 수집 (시작 전이거나 라이브 극초반이면 비어있을 수 있음)
                ...
                # 데이터가 아예 없는 경우 기본값 우회 처리 (안 튕기게 방어)
                if not stats_data or len(stats_data) < 2:
                    h_poss, h_shot, a_poss, a_shot = 50, 0, 50, 0
                    win_pick = "⚖️ 실시간 데이터 수집 대기 중"
                    control_pick = "경기 시작 전이거나 데이터를 분석 중입니다."
