...
# 💡 자체 데이터 생성 엔진 (최근 5경기 기반 전력 강제 추출)
def fetch_custom_team_stats(team_id):
    try:
        ...
        if not fixtures: return 10, 10, 10 # 최소 기본값
        ...
        att_score = min((goals_for / 15) * 100, 100) # 5경기 15골 기준 100점
        def_score = 100 - min((goals_against / 10) * 100, 100) # 실점이 적을수록 점수 높음
    ...
    for idx, league_id in enumerate(selected_leagues):
        ...
        try:
            ...
            for match in fixtures:
                ...
                # 💡 핵심: 데이터가 비어있다면 강제로 최근 전적을 긁어와 데이터 복원
                is_custom = False
                if sum(h_vals) < 10 or sum(a_vals) < 10:
                    ...
                    h_vals = [c_h_att, c_h_def, c_h_form, 50, c_h_att, 50] # 누락분 대체
                ...
                if odds_h > 0 and odds_a > 0:
                    if odds_h < odds_a - 0.3: # 홈 정배당
                        if a_power > h_power + 35: # 강제 수집 데이터 기반 더욱 엄격한 역배 조건
                            pred_winner, win_pick, pick_color = "away", f"🚨 데이터 추적 역배픽: {away_kr} 승", "#ff5252"
                        else:
                            pred_winner, win_pick, pick_color = "home", f"🟢 {home_kr} 승 (정배당)", "#00E676"
                    elif odds_a < odds_h - 0.3: # 원정 정배당
                        if h_power > a_power + 35: 
                            pred_winner, win_pick, pick_color = "home", f"🚨 데이터 추적 역배픽: {home_kr} 승", "#ff5252"
                        else:
                ...
                advice = translate_to_ko(pred_data['predictions'].get('advice', '자체 데이터 분석 완료'))
