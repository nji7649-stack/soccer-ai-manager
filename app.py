...
# API 키 설정 (API-Football과 API-Basketball은 동일한 키를 사용합니다)
API_KEY = st.secrets.get("FOOTBALL_API_KEY", "")
HEADERS = {'x-apisports-key': API_KEY} if API_KEY else {}

# 🎨 UI CSS (유령 공백 100% 제거 완료)
custom_css = """
<style>
                ...
                for event in games:
                    try:
                        ...
                        st.session_state['analyzed_data_list'].append({
                            ...
                            'stat_box': stat_box, 'referee': f"🏟️ {l_name} 경기장", 'p_h': f"{h_prob:.0f}", 'p_d': "0", 'p_a': f"{a_prob:.0f}", 
