...
# 🎨 UI CSS: 텍스트 1줄 강제 고정 (Line-clamp) 및 반응형 최적화
custom_css = """
<style>
...
/* 💡 핵심: 박스를 찢지 않고 끝부분을 ... 으로 잘라주는 마법의 CSS */
.ai-advice {
    font-size: 11.5px; 
    color: #999; 
    font-weight: normal; 
...
def fetch_custom_team_stats(team_id):
    try:
        url = "https://v3.football.api-sports.io/fixtures"
        res = requests.get(url, headers=HEADERS, params={"team": team_id, "last": 5}).json()
        ...
        for match in fixtures:
            ...
            if is_home:
                goals_for += h_goal
                goals_against += a_goal
                if h_goal > a_goal: wins += 1
            else:
                goals_for += a_goal
                goals_against += h_goal
                if a_goal > h_goal: wins += 1
                
        form_score = (wins / 5) * 100
        att_score = min((goals_for / 15) * 100, 100) 
        def_score = 100 - min((goals_against / 10) * 100, 100) 
        return att_score, def_score, form_score
    except:
        return 20, 20, 20
...
# 💡 마크다운 코드블록 버그 차단을 위해 HTML을 한 줄로 압축!
def create_html_radar(h_vals, a_vals, home_kr, away_kr, is_custom=False):
    ...
    custom_badge = "<div style='color:#ff9800; font-size:11px; margin-bottom:5px;'>⚙️ AI 자체 데이터 수집 가동</div>" if is_custom else ""

    html = f"<div style='display:flex; flex-direction:column; align-items:center; background:#0a0a0a; border:1px solid #333; border-radius:8px; padding:15px;'>{custom_badge}<div style='font-size:12px; color:#fff; margin-bottom:15px; font-weight:bold; text-align:center;'><span style='color:#4FC3F7;'>■</span> {home_kr} <span style='margin:0 10px; color:#777;'>vs</span> <span style='color:#EF5350;'>■</span> {away_kr}</div><svg viewBox='0 0 {size} {size}' style='width: 100%; max-width: {size}px; height: auto;'>{svg_lines}{h_poly}{a_poly}</svg></div>"
    return html
...
if st.sidebar.button("🚀 데이터 강제 수집 및 분석 시작"):
    ...
    for idx, league_id in enumerate(selected_leagues):
        ...
        try:
            ...
            for match in fixtures:
                ...
                try: 
                    match_time = datetime.strptime(match['fixture']['date'][:16], "%Y-%m-%dT%H:%M").strftime("%H:%M")
                except: 
                    match_time = "시간미정"
                ...
                if sum(h_vals) < 10 or sum(a_vals) < 10:
                    c_h_att, c_h_def, c_h_form = fetch_custom_team_stats(home_id)
                    c_a_att, c_a_def, c_a_form = fetch_custom_team_stats(away_id)
                ...
                if h_power < 10 or a_power < 10:
                    if odds_h > 0 and odds_a > 0:
                        if odds_h < odds_a: pred_winner, win_pick, pick_color = "home", f"🟢 {home_kr} 승 (데이터 부족/배당 기준)", "#00E676"
                        else: pred_winner, win_pick, pick_color = "away", f"🔵 {away_kr} 승 (데이터 부족/배당 기준)", "#4FC3F7"
                    else:
                        pred_winner, win_pick, pick_color = "none", "⚠️ 데이터/배당 모두 누락 (패스)", "#888888"
                else:
                    if odds_h > 0 and odds_a > 0:
                        if odds_h < odds_a - 0.3:
                            if a_power > h_power + 35: pred_winner, win_pick, pick_color = "away", f"🚨 데이터 추적 역배픽: {away_kr} 승", "#ff5252"
                            else: pred_winner, win_pick, pick_color = "home", f"🟢 {home_kr} 승 (정배당)", "#00E676"
                        elif odds_a < odds_h - 0.3:
                            if h_power > a_power + 35: pred_winner, win_pick, pick_color = "home", f"🚨 데이터 추적 역배픽: {home_kr} 승", "#ff5252"
                            else: pred_winner, win_pick, pick_color = "away", f"🔵 {away_kr} 승 (정배당)", "#4FC3F7"
                        else:
                ...
                stat_box += f"<span style='color:#aaa;'>전력 합산 점수:</span> {home_kr} <b>{int(h_power)}점</b> vs <b>{int(a_power)}점</b> {away_kr}"

                advice = translate_to_ko(pred_data['predictions'].get('advice', '데이터 분석 완료'))
                if is_custom: advice = "API 누락으로 인해 AI가 최근 전적을 자체 수집하여 전력을 계산했습니다."
                control_pick = f"{advice}"
                ...
                if under_over_val and not is_custom:
                    ...
                    if goal_expect >= 120: over_under = "🔥 2.5 기준 오버 (자체예측)"
                    else: over_under = "❄️ 2.5 기준 언더 (자체예측)"
    ...
    status_text.text("✅ 데이터 추적 및 분석 완료!")
    time.sleep(1)
    status_text.empty()
    progress_bar.empty()
...
# 💡 마크다운 코드블록 버그 완전 차단 (일렬 압축 결합)
if st.session_state.get('analyzed_data_list'):
    cols = st.columns(3)
    for idx, data in enumerate(st.session_state['analyzed_data_list']):
        with cols[idx % 3]:
            # 코드블록 방지를 위해 들여쓰기 없는 한 줄 HTML 문자열로 생성
            html_str = f"<div style='height: 100%;'>"
            html_str += f"<div class='card-box'>"
            html_str += f"<div><div class='league-txt'>{data['league']}</div>"
            html_str += f"<div class='match-txt'>{data['match_display']}</div>"
            html_str += f"<div class='stat-bg'>{data['stat_box']}</div></div>"
            html_str += f"<div class='predict-txt'>"
            html_str += f"<div style='color: {data['pick_color']}; margin-bottom: 3px;'>🎯 {data['win_pick']}</div>"
            html_str += f"<div class='over-under'>{data['over_under']}</div>"
            html_str += f"<div class='ai-advice'>⚔️ AI 요약: {data['control_pick']}</div>"
            html_str += f"</div></div></div>"
