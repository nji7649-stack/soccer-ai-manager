...
selected_leagues = st.sidebar.multiselect(
    ...
    default=["39", "140", "292"] # 기본적으로 EPL, 라리가, K리그 체크
)
        ...
        for league_id in selected_leagues:
            ...
            if fixtures:
                ...
                for match in fixtures:
                    ...
                    # 💡 클릭 시 펼쳐지는 아코디언 메뉴 (여기서 세부 분석!)
                    with st.expander(f"⚽ {home_kr} vs {away_kr} [{status}]"):
                        # 🚨 API 요금 폭탄 방지: '분석 실행' 버튼을 눌러야만 스탯을 가져옵니다.
                        if st.button(f"📊 '{home_kr} vs {away_kr}' 정밀 분석 실행", key=fix_id):
                            with st.spinner("라인업 및 세부 스탯 추출 중..."):
                                ...
                                if not stats_data or len(stats_data) < 2:
                                    st.info("아직 라인업/세부 스탯이 집계되지 않았습니다. (경기 전)")
                                else:
                                    ...
                                    control_pick = f"{home_kr} 압도적 주도" if h_poss > a_poss + 15 else (f"{away_kr} 압도적 주도" if a_poss > h_poss + 15 else "치열한 중원 싸움")
