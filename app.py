...
# 🎨 UI CSS (에러를 유발하던 사이드바 아이콘 해킹 코드 전면 삭제, 순수 레이아웃만 유지)
custom_css = """
<style>
            ...
            try:
                ...
                for match in (res.get('response') or []):
                    try:
                        ...
                        pred_is_over = True
                        if under_over_val:
                            if '-' in under_over_val: pred_is_over = False; ou_line = float(under_over_val.replace('-', '').replace('+', '').strip())
                            elif '+' in under_over_val: pred_is_over = True; ou_line = float(under_over_val.replace('+', '').replace('-', '').strip())
...
# ==========================================
# 🏀 농구 (무료 실시간 ESPN API - 핸디캡/언오버 및 라인업 100% 통합)
# ==========================================
