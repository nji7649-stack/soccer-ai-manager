...
if 'kbo_npb_data_list' not in st.session_state: 
    st.session_state['kbo_npb_data_list'] = []
if 'nba_upcoming_list' not in st.session_state: 
        ...
        if c_kbo or c_npb:
            ...
            for idx, (l_id, l_name) in enumerate(api_leagues):
                ...
                try:
                    ...
                    pass
        if not st.session_state['analyzed_data_list'] and not st.session_state['kbo_npb_data_list']: 
            st.info(f"해당 날짜({selected_date})에 한국시간 기준으로 야구 경기가 없습니다.")
        progress_bar.progress(1.0)
        status_text.text("✅ 야구 스캔 완료!")
        time.sleep(1.5)
        status_text.empty()
        progress_bar.empty()
    ...
    for idx, data in enumerate(st.session_state['kbo_npb_data_list']):
        with cols[idx % 3]:
            ...
            with st.expander("🔬 반자동 시뮬레이터 가동", expanded=True):
                ...
                h_name = data.get('home_kr', '홈')
                a_name = data.get('away_kr', '원정')
                ...
                if h_win_sim > a_win_sim + 5: 
                    win_pick = f"🟢 {h_name} 승리 유력"
                    pick_color = "#00E676"
                elif a_win_sim > h_win_sim + 5: 
                ...
                if total_exp > data['ou_line'] + 0.5: 
                    ou_text = f"🔥 총 {total_exp:.1f}점 (기준 {data['ou_line']} 오버)"
                elif total_exp < data['ou_line'] - 0.5: 
